...
if [ $mode = "-d" ]
then
  docker run -d --rm --name=k3s-client-09 $GPU2USE -u $(id -u):$(id -g) \
  ...
  /bin/bash -c "python -u -m nvflare.private.fed.app.client.client_train -m /workspace -s fed_client.json --set uid=k3s-client-09 secure_train=true config_folder=config org=nvidia"
else
  docker run --rm -it --name=k3s-client-09 $GPU2USE -u $(id -u):$(id -g) \
  -v /etc/passwd:/etc/passwd -v /etc/group:/etc/group -v $DIR/..:/workspace/ \
